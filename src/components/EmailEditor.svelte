<script lang="ts">
  ...
  const TEMPLATES = [
    { id: 'solicitud', name: 'Solicitud Recibida' },
  ...
  let selectedTemplateId = 'solicitud';

  let defaultSolicitud = `
<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaea; border-radius: 12px; overflow: hidden; background-color: #ffffff;">
  ...
  <div style="padding: 32px 24px;">
    <h2 style="color: #111827; margin-top: 0; font-size: 22px;">Solicitud Recibida, <strong>{{client_name}}</strong></h2>
    <p style="color: #4b5563; font-size: 16px; line-height: 1.5;">Hemos recibido tu solicitud de reserva para <strong>{{restaurant_name}}</strong>.</p>
    
    <div style="background-color: #fef3c7; border-left: 4px solid #f59e0b; padding: 16px; margin: 24px 0; border-radius: 0 8px 8px 0;">
      <h3 style="margin: 0 0 8px 0; font-size: 15px; color: #92400e;">Pendiente de confirmación</h3>
      <p style="margin: 0; color: #b45309; font-size: 14px; line-height: 1.5;">
        El restaurante está revisando tu solicitud para el día <strong>{{date}}</strong> a las <strong>{{time}}</strong> para <strong>{{pax}} personas</strong>.
      </p>
    </div>

    <p style="color: #4b5563; font-size: 16px; line-height: 1.5;">En breve recibirás un correo definitivo confirmando si disponemos de mesa o si no ha sido posible.</p>

    <hr style="border: none; border-top: 1px solid #eaeaea; margin: 24px 0;" />
    <p style="color: #6b7280; font-size: 14px; line-height: 1.5; margin: 0;">Si tus planes han cambiado y deseas retirar tu solicitud, puedes hacerlo aquí:</p>
    <div style="margin-top: 12px; margin-bottom: 24px;">
      <a href="https://delta-pala-booking-seven.vercel.app/cancelar/{{booking_id}}" style="background-color: #fce8e8; color: #dc2626; padding: 10px 16px; border-radius: 6px; text-decoration: none; display: inline-block; font-weight: bold; font-size: 13px; border: 1px solid #fecaca;">Cancelar Solicitud</a>
    </div>
    <p style="color: #6b7280; font-size: 14px; margin-top: 8px;"><em>¡Gracias por elegirnos!</em></p>
  </div>
</div>
  `;
